...
PHOENIX_NAMESPACE_BEGIN
    ...
    inline float SquareToGTR2Pdf(const Vector3f &m, float alpha) {
        float cosTheta = m.z() / m.norm();
        return (abs(m.norm() - 1) < EPSILON && m.z() >= 0) * pow(alpha, 2) * cosTheta * INV_PI /
               pow(1 + pow(cosTheta, 2) * (pow(alpha, 2) - 1), 2);
    }
    ...
    inline float SquareToGTR1Pdf(const Vector3f &m, float alpha) {
        float cosTheta = m.z() / m.norm();
        if (alpha >= 1)
            return INV_PI;
        return (abs(m.norm() - 1) < EPSILON && m.z() >= 0) * cosTheta * (pow(alpha, 2) - 1) * INV_PI /
               (2 * std::log(alpha) * (1 + pow(cosTheta, 2) * (pow(alpha, 2) - 1)));
    }
