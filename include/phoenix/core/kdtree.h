...
#ifndef PHOENIX_INCLUDE_PHOENIX_CORE_KDTREE_H_
#define PHOENIX_INCLUDE_PHOENIX_CORE_KDTREE_H_

#include<phoenix/core/phoenix.h>
#include<phoenix/core/bbox.h>
#include<iostream>
#include<algorithm>
#include<tbb/tbb.h>
...
template <typename _NodeType> class PointKDTree {
 ...
 public:
  ...
  void build(bool recomputeBoundingBox = false) {
    if (m_nodes.size() == 0) {
      std::cerr << "KDTree::build(): kd-tree is empty!" << std::endl;
      return;
    }

    std::cout << "Building a " << Dimension << "-dimensional kd-tree over "
...
#endif //PHOENIX_INCLUDE_PHOENIX_CORE_KDTREE_H_
