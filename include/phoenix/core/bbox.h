...
#ifndef PHOENIX_INCLUDE_PHOENIX_CORE_BBOX_H_
#define PHOENIX_INCLUDE_PHOENIX_CORE_BBOX_H_
  ...
  bool rayIntersect(const Ray &ray) const {
    ...
    for (int i=0; i<3; i++) {
      ...
      if (ray.dir_[i] == 0) {
        ...
        float t1 = (minVal - origin) * ray.dir_.cwiseInverse()[i];
        float t2 = (maxVal - origin) * ray.dir_.cwiseInverse()[i];
    ...
    return ray.mint_ <= farT && nearT <= ray.maxt_;
  }
...
#endif //PHOENIX_INCLUDE_PHOENIX_CORE_BBOX_H_
