...
class TestemsIntegrator : public Integrator {
 private:
  float russian_;
 public:
  explicit TestemsIntegrator(const PropertyList &props) {
  ...
  [[nodiscard]] Color3f Li(shared_ptr<Scene> scene, shared_ptr<Sampler> sampler, const Ray &ray) const override {
    ...
    if (sampler->Next1D() <= russian_)
      return res+ Li(scene, sampler, light_ray) * bsdfvalue * cos0 / russian_;
...
PHOENIX_REGISTER_CLASS(TestemsIntegrator, "testems");
