...
class AreaLight : public Emitter {
 ...
 public:
  ...
  Color3f SamplePhoton(Ray &ray, shared_ptr<Sampler> sampler) const override {
    float pdf = 0.0f;

    SampleData shape_sample = shape_->SampleSurface(sampler->Next2D());

    Vector3f wi = SquareToCosineHemisphere(sampler->Next2D());

    Frame frame(shape_sample.normal);

    Vector3f xfm_wi = frame.ToWorld(wi);

//    pdf = SquareToCosineHemispherePdf(wi) / shape_sample.pdf;

    ray = Ray(shape_sample.point, xfm_wi);

    return PI * shape_->GetArea() * radiance;
